...
export function parseAgentOutput<T extends { findings: unknown[] }>(
  ...
  if (!parsed.success) {
    ...
    if (!retry.success) {
      warn(`[codec] schema validation failed: ${retry.error.message}`);
      return {
        output: { findings: [] } as unknown as T,
        meta: {
          status: 'invalid_output',
    ...
    log(
      `[codec] parsed ${retry.data.findings.length} findings (after normalization)`,
    );
    return { output: retry.data, meta: { status: 'ok' } };
  }

  log(`[codec] parsed ${parsed.data.findings.length} findings`);
